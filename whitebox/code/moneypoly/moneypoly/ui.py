...
def print_player_card(player):
    ...
    if player.jail["in_jail"]:
        jail_line = f"  Status  : IN JAIL (turn {player.jail['turns']}/3)\n"
    print(f"\n  Player  : {player.name}")
    print(f"  Balance : ${player.balance:,}")
    print(f"  Worth   : ${player.net_worth():,}")
    print(f"  Position: {player.position}")
    print(jail_line, end="")
    if player.jail["cards"]:
        print(f"  Jail cards: {player.jail['cards']}")
...
def print_standings(players):
    ...
    for i, player in enumerate(ranked, start=1):
        jail_tag = " [JAILED]" if player.jail["in_jail"] else ""
        print(
            f"  {i}. {player.name:<16} "
            f"${player.balance:>6,}  "
